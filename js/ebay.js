document.addEventListener("DOMContentLoaded", () => {
    ...
    const marketplaceConfig = {
        GB: { name: "United Kingdom", flag: "🇬🇧", currency: "GBP", rate: 3600 },
        DE: { name: "Germany", flag: "🇩🇪", currency: "EUR", rate: 3200 },
        FR: { name: "France", flag: "🇫🇷", currency: "EUR", rate: 3200 },
        IE: { name: "Ireland", flag: "🇮🇪", currency: "EUR", rate: 3200 }
    };
    ...
    const createMarketplaceSummary = (items) => {
        ...
        element.innerHTML = `
            <div class="summary-content">
                <h3>${productTypes[productTypeFilter.value]?.icon || '🎮'} Found ${items.length.toLocaleString()} ${getProductTypeName()} Listings</h3>
                <p>${summaryText}</p>
            </div>`;
    ...
    const createProductCard = (item) => {
        ...
        const sellerRating = parseFloat(item.feedbackPercentage);
        ...
        <span class="condition-badge">${item.condition}</span>
            ...
            <div class="game-card-content">
                ...
                <div class="game-price">
                    <strong>Price:</strong> ${formatPrice(item.originalPrice)}
                </div>
                <div class="game-price-mwk">
                    <strong>Price (MWK):</strong> ${formatPrice(item.priceMWK)}
                </div>
                ...
                <div class="seller-info">
                    ...
                    <span style="color: ${ratingColor};">${item.feedbackPercentage}%</span>
                </div>
    ...
    const sortItems = (items, sortOrder) => {
        return [...items].sort((a, b) => {
            const priceA = parseFloat(a.originalPrice.split(" ")[0]);
            const priceB = parseFloat(b.originalPrice.split(" ")[0]);
    ...
    const loadEbayListings = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:3000/api/ebay/items?${params.toString()}`);
